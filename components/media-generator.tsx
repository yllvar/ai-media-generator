...
export default function MediaGenerator() {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      console.log("Starting video generation request with prompt:", prompt)
      ...
      if (!response.ok) {
        ...
        const errorDetails = data.details ? `: ${data.details}` : ""
        console.error(`Error generating video: ${errorMessage}${errorDetails}`)
      ...
      setGenerationStatus("Complete")
      setProgressPercent(100)
      console.log("Successfully set video with requestId:", data.requestId)
    } catch (err) {
      ...
      <Card className="w-full">
        ...
        <CardContent className="pt-6">
          <Tabs value={activeTab} onValueChange={handleTabChange} className="mb-6">
            ...
            <TabsContent value="image" className="mt-4">
              <p className="text-sm text-gray-500 mb-4">
                Generate images using Stable Diffusion XL via Replicate. Create detailed, high-quality images from text
                prompts.
              </p>
            </TabsContent>
            <TabsContent value="video" className="mt-4">
              <p className="text-sm text-gray-500 mb-4">
                Generate videos using Lightricks/LTX-Video via Hugging Face. Create short video clips from text prompts.
              </p>
            </TabsContent>
          </Tabs>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex flex-col space-y-2">
              ...
              <div className="flex space-x-2">
                <Input
                  ...
                  placeholder={
                    activeTab === "image"
                      ? "Astronaut riding a horse, photorealistic, detailed, 8K"
                      : "A young man walking on the street, cinematic lighting"
                  }
                  className="flex-1"
                  disabled={loading}
                />
            ...
            {error && (
              <div className="p-4 bg-red-50 border border-red-200 text-red-600 rounded-md">
                <div className="flex items-start">
                  ...
                  <div>
                    <h4 className="font-medium">Error generating {activeTab}</h4>
            ...
            {/* Display generated content based on active tab */}
            {activeTab === "image" && image && !loading && (
            ...
            {activeTab === "video" && video && !loading && (
              <div className="mt-6 space-y-2">
                <h3 className="text-lg font-medium">Generated Video</h3>
                <VideoPlayer src={video} className="w-full aspect-video" />
                <div className="flex justify-between items-center">
                  <p className="text-sm text-gray-500">Prompt: {prompt}</p>
                  {requestId && <p className="text-xs text-gray-400">Request ID: {requestId}</p>}
                </div>
